...
const ProductDetails: FC<IProductDetails> = ({ id_product }) => {
    ...
    const links = ['www.facebook.com', 'www.x.com', 'www.instagram.com', 'www.pinterest.com'];
    ...
    const handleMouseMove: MouseEventHandler<HTMLDivElement> = (event) => {
        ...
        if (imgSlide && containerImgSlideRef) {
            ...
            const isMouseOverImage = event.target === imgSlide.current || imgSlide.current.contains(event.target as Node);
    ...
    const handleClickDetailsProductContainer: MouseEventHandler<HTMLDivElement> = (event) => {
        event.stopPropagation();
    }

    return (
        <BGContainer className={`d-flex ${actived ? 'actived' : ''}`} onClick={handleClickCloseBG}>
            <ProductDetailsContainer className="d-flex" onClick={handleClickDetailsProductContainer}>
                        ...
                        (pathSlideImages.length > 0) && (
                            <SlideContainer className="d-flex">
                                <Slide className="d-grid">
                                    {
                                        pathSlideImages.map((path) => (<ItemSlide pathImageProdc={path} theRef={imgSlide} key={uuid()} />))
                                    }
                                </Slide>
                            </SlideContainer>
                        )
                ...
                <ContainerDetails className="d-flex">
                        ...
                        <O_Frame220 className="d-flex">
                            ...
                            <O_Div0 className="d-flex">
                                <O_SpanSku className="d-flex" title="Avaliações por pessoa">
                                    AP: <O_BoldSku>2,51,594</O_BoldSku>
                                </O_SpanSku>
                            </O_Div0>
                        </O_Frame220>
                        ...
                        <Frame222 className="d-flex">
                            ...
                            <ShareContainer className="d-flex">
                                ...
                                <Share className="d-flex">
                                    <ShareWith url={links[0]} Icon={<Facebook02Icon strokeWidth={0.1} fill="var(--Gray-700)" size={17} className="svg-icon" />} />
                                    <ShareWith url={links[1]} Icon={<TwitterIcon strokeWidth={0.1} fill="var(--Gray-700)" size={17} className="svg-icon" />} />
                                    <ShareWith url={links[2]} Icon={<FaPinterestP color="var(--Gray-700)" className="svg-icon-stroke" />} />
                                    <ShareWith url={links[3]} Icon={<InstagramIcon color="var(--Gray-700)" strokeWidth={2} size={17} className="svg-icon-stroke" />} />
                                </Share>
                            </ShareContainer>
                        </Frame222>
                        <Frame224>
                            <TextFrame224>Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Nulla nibh diam, blandit vel consequat nec, ultrices et ipsum. Nulla varius magna a consequat pulvinar.</TextFrame224>
                        </Frame224>
                    </Frame223>
                    <Divider color="var(--Gray-800)" sx={{ width: "100%" }} />
                    ...
                    <SmallDetails className="d-flex">
                        ...
                        <Category className="d-flex">
                            ...
                            <BaseTextcategory>vegetal</BaseTextcategory>
                            <BoldCategoryText>angola</BoldCategoryText>
                            <BaseTextcategory>Vegetal</BaseTextcategory>
                        </Category>
                    </SmallDetails>
                </ContainerDetails>
