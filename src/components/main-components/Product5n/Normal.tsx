...
interface INormal extends React.DetailedHTMLProps<React.HTMLAttributes<HTMLDivElement>, HTMLDivElement> {
  ratingNumber?: number;
  saleValue?: number;
  price: number;
...
const Normal: FC<INormal> = ({ ratingNumber = 1, saleStatus = [], saleValue = undefined, price, btnBuyProductEvent, productId, ...rest }) => {
  const sizeStar = 15;
  const size = 25;
    ...
    <div className="container-normal product5n" data-id={productId} {...rest}>
      ...
      <div className="container-product-info d-flex">
        ...
        <div className="container-add-to-cart">
          <button type="button" className="btn-add-to-cart d-flex" title="Adicionar ao carrinho" onClick={btnBuyProductEvent} data-id={productId}>
